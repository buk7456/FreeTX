...
void controlBacklight()
{
  ...
  if(Sys.backlightWakeup == BACKLIGHT_WAKEUP_KEYS)
  {
    ...
    //When the backlight is on, treat all keys the same.
    static bool inhibit = false;
    if(!backlightIsOn && buttonCode >= KEY_TRIM_FIRST && buttonCode <= KEY_TRIM_LAST)
      inhibit = true;
    if(buttonCode == KEY_UP || buttonCode == KEY_DOWN || buttonCode == KEY_SELECT)
      inhibit = false;
    if(inhibit)
      elapsed = duration;
